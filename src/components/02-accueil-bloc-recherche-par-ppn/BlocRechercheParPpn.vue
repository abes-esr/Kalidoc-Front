...
export default {
  ...
  data() {
    return {
        ...
        lastValuesTypedOrPasted: '', //Dernière Chaîne de caractères saisie dans la combobox, servant à alimenter ensuite ppnListTyped
        ppnListCombobox: [], //Tableau de ppn alimenté par les chaînes de caractères mises dans la combobox
  ...
  methods: {
    ...
    checkValuesAndFeedPpnListTyped(){
      if(!!this.lastValuesTypedOrPasted){ //Si la valeur n'est pas nulle, ce qui se produit si l'utilisateur sort du cadre sans rien taper
        let arrayWithValidsPpn = this.lastValuesTypedOrPasted.split(/[^Xx\d]/).filter(ppn_to_check => ppn_to_check.match(/^(\d{8}(\d|X|x))$/));
        let arrayWithInvalidsPpn = this.lastValuesTypedOrPasted.split(/[^Xx\d]/).filter(ppn_to_check => !ppn_to_check.match(/^(\d{8}(\d|X|x))$/)).filter(str_to_clean => str_to_clean.trim() !== '');
        ...
        arrayWithValidsPpnWithUniqueValues.forEach(currentValidPpn => this.ppnListCombobox.push(currentValidPpn));
      }
      this.lastValuesTypedOrPasted = ''; //On vide la chaîne puisqu'on à alimenté les valeurs valides dans :value="ppnListCombobox"
    },
  }
};
</script>
